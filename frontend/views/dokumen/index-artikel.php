...
$this->title = 'Koleksi Digital - JDIH UPNVJT';
...
$jenisList = ['' => 'Semua Jenis', 'ARTIKEL' => 'Artikel', 'MAJALAH HUKUM' => 'Majalah Hukum', 'JURNAL HUKUM' => 'Jurnal Hukum', 'SKRIPSI' => 'Skripsi', 'TESIS' => 'Tesis'];
...
<section class="peraturan-hero">
    <div class="breadcrumb"><a href="<?= Url::to(['/site/index']) ?>">Beranda</a> &nbsp;&gt;&nbsp;<span class="current">Koleksi Digital</span></div>
    <h1>Koleksi Digital</h1>
    <p>Arsip digital artikel, jurnal, dan koleksi hukum dalam lingkungan UPN Veteran Jawa Timur.</p>
</section>

<section class="peraturan-search">
    <form action="<?= Url::to(['/dokumen/artikel']) ?>" method="get" style="display:flex;gap:12px;width:100%;">
        <div class="peraturan-search-input">
            <i class="bi bi-search" aria-hidden="true"></i>
            <input type="text" name="DokumenSearch[judul]" placeholder="Cari koleksi digital..." value="<?= Html::encode($currentKeyword) ?>">
        </div>
...
<nav class="peraturan-tabs">
    ...
    <a href="<?= Url::to(['/dokumen/artikel']) ?>" class="active">Koleksi Digital</a>
</nav>

<div class="peraturan-content">
    <aside class="peraturan-filter">
        <h3>Filter</h3>
        <form action="<?= Url::to(['/dokumen/artikel']) ?>" method="get">
            <div class="peraturan-filter-group">
                <label class="group-title">Jenis Koleksi</label>
    ...
    <section class="peraturan-results">
        <div class="peraturan-toolbar">
            <span>Menampilkan <strong><?= number_format($from) ?>-<?= number_format($to) ?></strong> dari <strong><?= number_format($totalCount) ?></strong> koleksi</span>
        </div>
        <?php if ($totalCount === 0): ?>
            <div class="peraturan-empty">
                ...
                Tidak ada koleksi digital ditemukan.
            </div>
        <?php else: ?>
            <?= ListView::widget([
