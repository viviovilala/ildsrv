...
$this->title = 'Yurisprudensi - JDIH UPNVJT';
...
$jenisList = ['' => 'Semua Jenis', 'PUTUSAN MAHKAMAH AGUNG' => 'Putusan Mahkamah Agung', 'PUTUSAN MA' => 'Putusan MA', 'PUTUSAN PTUN' => 'Putusan PTUN'];
$tahunList = []; for ($y = (int) date('Y'); $y >= 2000; $y--) { $tahunList[$y] = $y; }
$statusList = ['' => 'Semua Status', 'Berlaku' => 'Berlaku', 'Tidak Berlaku' => 'Tidak Berlaku', 'Dicabut' => 'Dicabut'];
...
<section class="peraturan-hero">
    <div class="breadcrumb"><a href="<?= Url::to(['/site/index']) ?>">Beranda</a> &nbsp;&gt;&nbsp;<span class="current">Yurisprudensi</span></div>
    <h1>Yurisprudensi</h1>
    <p>Putusan pengadilan dan hasil uji materi dalam lingkungan UPN Veteran Jawa Timur.</p>
</section>

<section class="peraturan-search">
    <form action="<?= Url::to(['/dokumen/putusan']) ?>" method="get" style="display:flex;gap:12px;width:100%;">
        <div class="peraturan-search-input">
            <i class="bi bi-search" aria-hidden="true"></i>
            <input type="text" name="DokumenSearch[judul]" placeholder="Cari yurisprudensi..." value="<?= Html::encode($currentKeyword) ?>">
        </div>
...
<nav class="peraturan-tabs">
    ...
    <a href="<?= Url::to(['/dokumen/putusan']) ?>" class="active">Yurisprudensi</a>
...
<div class="peraturan-content">
    <aside class="peraturan-filter">
        <h3>Filter</h3>
        <form action="<?= Url::to(['/dokumen/putusan']) ?>" method="get">
            <div class="peraturan-filter-group">
                <label class="group-title">Jenis Putusan</label>
    ...
    <section class="peraturan-results">
        <div class="peraturan-toolbar">
            <span>Menampilkan <strong><?= number_format($from) ?>-<?= number_format($to) ?></strong> dari <strong><?= number_format($totalCount) ?></strong> yurisprudensi</span>
        </div>
        <?php if ($totalCount === 0): ?>
            <div class="peraturan-empty">
                ...
                Tidak ada yurisprudensi ditemukan.
            </div>
        <?php else: ?>
            <?= ListView::widget([
