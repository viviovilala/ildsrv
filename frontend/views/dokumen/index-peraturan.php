...
.peraturan-hero .breadcrumb a { color: rgba(255,255,255,.75); }
.peraturan-hero .breadcrumb .current { color: #d4a94a; font-weight: 600; }
...
.peraturan-search {
    display: flex; gap: 12px; max-width: 1280px; margin: 24px auto 0; padding: 0 0;
}
.peraturan-search-input {
    flex: 1; display: flex; align-items: center; gap: 10px;
    background: #fff; border: 1px solid #e3ded0; border-radius: 24px; padding: 12px 20px;
}
...
.peraturan-search-btn {
    background: #1e4620; color: #fff; border: none; border-radius: 24px;
    padding: 0 28px; font-size: 15px; font-weight: 600; cursor: pointer;
}

.peraturan-tabs {
    display: flex; gap: 0; max-width: 1280px; margin: 24px auto 0;
    border-bottom: 1px solid #e3ded0; font-size: 14px;
}
.peraturan-tabs a {
    padding: 12px 24px; color: #6b6f6b; border-bottom: 2px solid transparent;
...
.peraturan-content {
    display: flex; gap: 32px; max-width: 1280px; margin: 28px auto 60px;
    align-items: flex-start;
}
.peraturan-filter {
    width: 250px; flex-shrink: 0; background: #fff; border: 1px solid #e3ded0;
...
.peraturan-filter select {
    width: 100%; padding: 10px 12px; border: 1px solid #e3ded0; border-radius: 8px;
    font-size: 13px; background: #fff; color: #1f2a1f;
}
.peraturan-filter-btn {
    width: 100%; background: #1e4620; color: #fff; border: none; border-radius: 8px;
...
.peraturan-toolbar {
    ...
    margin-bottom: 16px; font-size: 14px; color: #6b6f6b;
}
.peraturan-toolbar strong { color: #1f2a1f; }

.doc-card {
    background: #fff; border: 1px solid #e3ded0; border-radius: 10px;
...
.doc-card h3 a { color: #1f2a1f; }
.doc-card h3 a:hover { color: #1e4620; text-decoration: underline; }
...
.doc-card-detail {
    ...
    background: #f7f4ec; border: 1px solid #e3ded0; border-radius: 20px;
    padding: 8px 18px; font-size: 13px; font-weight: 600; color: #1f2a1f;
    white-space: nowrap; transition: background .15s ease;
}
.doc-card-detail:hover { background: #e3ded0; text-decoration: none; color: #1f2a1f; }

.peraturan-empty {
    text-align: center; padding: 60px 20px; color: #6b6f6b;
    background: #fff; border: 1px dashed #e3ded0; border-radius: 10px;
}

.peraturan-pagination { display: flex; justify-content: center; align-items: center; gap: 6px; margin-top: 28px; }
.peraturan-pagination a, .peraturan-pagination span {
    display: inline-flex; align-items: center; justify-content: center;
    min-width: 36px; height: 36px; border-radius: 50%; font-size: 14px; color: #1f2a1f;
}
.peraturan-pagination a.active { background: #1e4620; color: #fff; }
.peraturan-pagination a:hover:not(.active) { background: #e3ded0; }
.peraturan-pagination .nav-arrow { border: 1px solid #e3ded0; }

@media (max-width: 900px) {
    .peraturan-content { flex-direction: column; }
    .peraturan-filter { width: 100%; }
    .peraturan-hero { padding: 28px 24px 36px; }
    .peraturan-hero h1 { font-size: 28px; }
...
<!-- HERO -->
<section class="peraturan-hero" style="background: #154212;">
    <div class="breadcrumb">
        <a href="<?= Url::to(['/site/index']) ?>">Beranda</a> &nbsp;&gt;&nbsp;
        <span class="current">Produk Hukum</span>
    </div>
    <h1>Produk Hukum</h1>
    <p>Arsip digital peraturan dan keputusan hukum dalam lingkungan UPN Veteran Jawa Timur.</p>
</section>

<!-- SEARCH -->
<section class="peraturan-search">
    <form action="<?= Url::to(['/dokumen/peraturan']) ?>" method="get" style="display:flex; gap:12px; width:100%;">
        <div class="peraturan-search-input">
            <i class="bi bi-search" aria-hidden="true"></i>
            <input type="text" name="DokumenSearch[judul]" placeholder="Cari produk hukum..." value="<?= Html::encode($currentKeyword) ?>">
        </div>
...
<nav class="peraturan-tabs">
    <a href="<?= Url::to(['/dokumen/peraturan']) ?>" class="active">Peraturan</a>
    <a href="<?= Url::to(['/dokumen/putusan']) ?>">Yurisprudensi</a>
    <a href="<?= Url::to(['/dokumen/monografi']) ?>">Monografi</a>
...
<div class="peraturan-content">
    ...
    <aside class="peraturan-filter">
        <h3>Filter</h3>
        <form action="<?= Url::to(['/dokumen/peraturan']) ?>" method="get">

            <div class="peraturan-filter-group">
                <label class="group-title">Jenis Peraturan</label>
    ...
    <section class="peraturan-results">
        <div class="peraturan-toolbar">
            <span>Menampilkan <strong><?= number_format($from) ?>-<?= number_format($to) ?></strong> dari <strong><?= number_format($totalCount) ?></strong> produk hukum</span>
        </div>
        ...
        <?php else: ?>
            <?= ListView::widget([
                ...
                'summary' => false,
                'pager' => [
                    'options' => ['class' => 'peraturan-pagination'],
                    'pageCssClass' => '',
                    'linkOptions' => ['class' => ''],
