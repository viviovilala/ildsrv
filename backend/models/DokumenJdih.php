 ...
 *
 * @property DataLampiran[] $dataLampirans
 */
class DokumenJdih extends \yii\db\ActiveRecord
{
    ...
    public function getLampiran($id)
    {
        if (!empty($id)) {
            //return 'ada';
            return Html::a($id, ['../common/dokumen/' . $id], ['target' => '_blank', 'data-pjax' => 0, 'title' => 'lihat file']);
        }
    }

    public function getJenis($id)
    {
        $jenis = JenisPeraturan::findOne($id);
        return $jenis->name;
    }
            ...
            if (!empty($catatan)) {
                ...
                if ($catatan->created_by == \Yii::$app->user->identity->id) {
                    return Html::a(strtoupper($dokumen->judul), ['view', 'id' => $dokumen->id]) . '<br>' .
                        "<span class='label label-danger'>verifikator :" . $catatan->catatan . "</span>";
                } else {
                    return Html::a(strtoupper($dokumen->judul), ['view', 'id' => $dokumen->id]) . '<br>' . "<span class='label label-success'>operator :" . $catatan->catatan . "</span>";
                }
            }
    ...
    {
        return $this->hasMany(Eksemplar::className(), ['id_dokumen' => 'id']);
    }

    public function getSubjek()
    {
        return $this->hasMany(DataSubyek::className(), ['id_dokumen' => 'id']);
    }
    ...
    {
        return $this->hasOne(TipeDokumen::className(), ['id_dokumen' => 'id']);
    }


    public function getSubyek2($id)
    {
        $return = '';
        $eksempar = DataSubyek::find()->where(['id_dokumen' => $id])->all();
        if (!empty($eksempar)) {
            foreach ($eksempar as $data) {
                $return = $return . $data->subyek . '<br>';
            }
        }
        return $return;
    }
    ...
    {
        $return = '';
        $eksempar = Eksemplar::find()->where(['id_dokumen' => $id])->all();
        if (!empty($eksempar)) {
            foreach ($eksempar as $data) {
                $return = $return . Html::a($data->kode_eksemplar, ['/eksemplar/view', 'id' => $data->id], ['class' => 'text-primary', 'title' => 'lihat detail']) . '<br>';
            }
        }
        return $return;
    }
