...
class ProgramController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        /*
        $searchModel = new ProgramSearch(['id'=>\Yii::$app->user->identity->direktorat_id]);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            ...
            */
            ...
            if ($model->save()) 
            {
                Yii::$app->session->setFlash('success', 'Data Program berhasil ditambahkan');
                return $this->redirect(['view', 'id' => $model->id]);
            } else 
            {
                Yii::$app->session->setFlash('error', 'Data Program Gagal ditambahkan, periksa kembali ');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Data Program berhasil diubah');
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
    ...
    public function actionDelete($id)
    {
        try
        {
            ...
            Yii::$app->session->setFlash('danger', 'Data Program berhasil dihapus');
            return $this->redirect(['index']);
        }
        catch(\yii\db\IntegrityException  $e)
        {
            Yii::$app->session->setFlash('error', "Data Program Tidak Dapat Dihapus Karena Dipakai Modul Lain");
            return $this->redirect(['index']);
        } 
