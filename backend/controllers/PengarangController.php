...
class PengarangController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            if ($model->save()) 
            {
                Yii::$app->session->setFlash('success', 'Data Pengarang berhasil ditambahkan');
                return $this->redirect(['view', 'id' => $model->id]);
            } else 
            {
                Yii::$app->session->setFlash('error', 'Data Pengarang Gagal ditambahkan, periksa kembali ');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Data Pengarang berhasil diubah');
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
    ...
    public function actionDelete($id)
    {
        ...
        {
            Yii::$app->session->setFlash('error', "Data Pengarang Tidak Dapat Dihapus Karena Dipakai Modul Lain");
            return $this->redirect(['index']);
        } 
