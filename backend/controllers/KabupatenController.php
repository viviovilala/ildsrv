...
class KabupatenController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        /*
        $searchModel = new KabupatenSearch(['id'=>\Yii::$app->user->identity->direktorat_id]);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            ...
            */
            ...
            if ($model->save()) 
            {
                Yii::$app->session->setFlash('success', 'Data Kabupaten berhasil ditambahkan');
                return $this->redirect(['view', 'id' => $model->id]);
            } else 
            {
                Yii::$app->session->setFlash('error', 'Data Kabupaten Gagal ditambahkan, periksa kembali ');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Data Kabupaten berhasil diubah');
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
    ...
    public function actionDelete($id)
    {
        try
        {
            ...
            Yii::$app->session->setFlash('danger', 'Data Kabupaten berhasil dihapus');
            return $this->redirect(['index']);
        }
        catch(\yii\db\IntegrityException  $e)
        {
            Yii::$app->session->setFlash('error', "Data Kabupaten Tidak Dapat Dihapus Karena Dipakai Modul Lain");
            return $this->redirect(['index']);
        } 
